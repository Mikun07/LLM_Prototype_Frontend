    ...
    <div>
      <label
        ...
        className={[
          ...
          isUploading ? 'cursor-wait border-brand-300 bg-white/90' : '',
        ].join(' ')}
        onDragLeave={onDragLeave}
        onDragOver={onDragOver}
        onDrop={onDrop}
      >
        <div className={['flex h-16 w-16 items-center justify-center rounded-2xl shadow-lg shadow-brand-200/50 transition-all', isDragging || isUploading ? 'scale-110 bg-brand-600 text-white' : 'bg-gradient-brand text-white'].join(' ')}>
          {isUploading ? (
        ...
        <div>
          <span className="block text-lg font-bold text-slate-800">
            {isUploading ? 'Uploading and validating CSV' : 'Upload requirements CSV'}
          </span>
          <span className="mt-1 block max-w-sm text-sm leading-6 text-slate-500">
            {isUploading ? (
              'The backend is parsing the file and detecting requirement columns.'
            ) : (
              <>
